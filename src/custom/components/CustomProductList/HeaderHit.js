...
import {Highlight, Link} from "react-instantsearch-dom";
...
// This is a temporary cartItem object to be used for testing pending the
// GraphQL endpoint being hooked up Remove the code between these comments when
...
@inject("uiStore")
@observer
class HeaderHit extends React.Component {
    static propTypes = {
        /**
     ...
     */
        ...
        uiStore: PropTypes
            .shape({openCartWithTimeout: PropTypes.func})
            .isRequired,
   ...
   */
    handleAddToCartClick = async (quantity) => {
        ...
        const selectedOption = selectedVariant.options && variantById(
        ...
        if (selectedVariantOrOption) {
            // Get the price for the currently selected variant or variant option const
            // price = priceByCurrencyCode(currencyCode, selectedVariantOrOption.pricing);
            const price = product['price'];

            // Call addItemsToCart with an object matching the GraphQL `CartItemInput`
            // schema
            let dataGraph = {
                price: {
                    //amount: price.price,
            ...
            const {data} = await addItemsToCart([dataGraph]);
            ...
            if (data) {
                ...
                // The mini cart popper will open automatically after adding an item to the
                // cart, therefore, a CART_VIEWED event is published. debugger
                // eslint-disable-line
    ...
    render() {
        ...
        return (
            <div className="product">
                <a href={`/product/${product['slug']}/${product['product-id']}`}>
